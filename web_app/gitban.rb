...
get '/' do
  @to_do_issues=Issue.joins(assignee: :stage).where('stages.name=? and issues.state=?', 'READY','open')
  @in_progress_issues=Issue.joins(assignee: :stage).where('(stages.name=? OR stages.name=?) and issues.state=?' , 'DEV','UI','open')
  @qa_issues=Issue.joins(assignee: :stage).where('stages.name=? and issues.state=?', 'QA','open')
  @uat_issues=Issue.joins(assignee: :stage).where('stages.name=? and issues.state=?' , 'UAT','open')
